...
# For the regression suite: pip install -r requirements-dev.txt
```
...
| `BOT_DATABASE_URL` | ❌        | `sqlite+aiosqlite:///./data/…`      | Bot-specific SQLAlchemy async connection URL   |
...
| `CHECKOUT_SESSION_TIMEOUT_MINUTES` | ❌ | `15` | Server-side package checkout session lifetime |
| `ORDER_PAYMENT_TIMEOUT_MINUTES` | ❌ | `30` | Unpaid order payment deadline                  |
...
```
 ...
 6. Initialise service registry (SettingsService, LanguageService, UserService, PackageCatalogService, OrderService, CheckoutService, …)
...
10. Register middlewares (request_context @ -4; auth @ -3; language @ -2; activity @ -1)
11. Register handlers (start, customer navigation, account, package catalogue/checkout/orders, keys, admin)
