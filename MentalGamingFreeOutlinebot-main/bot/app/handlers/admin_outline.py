...
from app.keyboards.admin_outline_setup import outline_not_found_keyboard, outline_review_keyboard, outline_setup_methods_keyboard, ssh_auth_keyboard, ssh_test_keyboard
...
async def outline_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(parts) == 4 and parts[2] == "future":
        await message.reply_text(t("admin.outline.coming_soon", language=language, feature="Auto-Provision" if parts[3] == "auto" else "SSH"), reply_markup=outline_setup_methods_keyboard(language)); return
    if len(parts) == 4 and parts[2] in {"cancel", "test_again", "save_disabled", "save_enable"}:
        ...
        if parts[2] == "cancel":
            ...
            service.cancel_setup(admin_id=actor, flow_id=flow_id); context.user_data.pop(_STATE, None); await message.reply_text(t("admin.outline.cancelled", language=language), reply_markup=outline_setup_methods_keyboard(language)); return
