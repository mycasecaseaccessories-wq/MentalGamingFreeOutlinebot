...
class Settings:
    ...
    """Maximum automatic retries for transient operations."""

    checkout_session_timeout_minutes: int = field(
        default_factory=lambda: _parse_int("CHECKOUT_SESSION_TIMEOUT_MINUTES", 15)
    )
    """Lifetime of a server-side checkout selection in minutes."""

    order_payment_timeout_minutes: int = field(
        default_factory=lambda: _parse_int("ORDER_PAYMENT_TIMEOUT_MINUTES", 30)
    )
    """Payment deadline for an unpaid order in minutes."""
