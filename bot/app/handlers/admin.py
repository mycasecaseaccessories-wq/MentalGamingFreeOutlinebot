...
def _authorization_service(context: ContextTypes.DEFAULT_TYPE) -> AdminAuthorizationService | None:
    registry = context.bot_data.get("registry")
    return None if registry is None else registry.get_or_none(AdminAuthorizationService)
...
def _item_text(item: PaymentReviewItem, language: str) -> str:
    return cast("str", t(
        ...
        proof=t(
            "admin.payments.proof_attached"
            if item.proof_file_id
            else "admin.payments.proof_missing",
            language=language,
        ),
    ))
...
async def _show_payment_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if message:
        await message.reply_text(
            t("admin.payments.menu", language=_language(context)),
            reply_markup=admin_payment_menu_keyboard(_language(context)),
        )
...
async def admin_payments_callback(  # noqa: PLR0911, PLR0912, PLR0915
    ...
    try:
        ...
        if len(parts) == 4 and parts[:3] == ["admin", "payments", "pending"]:
            ...
            if not result.items:
                await message.reply_text(
                    t("admin.payments.pending_empty", language=language),
                    reply_markup=admin_payment_menu_keyboard(language),
                )
                return
            pages = max(1, math.ceil(result.total / result.page_size))
            text = t(
                "admin.payments.queue_header", language=language, page=result.page, pages=pages
            )
            ...
            await message.reply_text(
                text,
                reply_markup=admin_pending_queue_keyboard(
                    result.page,
                    result.has_previous,
                    result.has_next,
                    language,
                    payment_ids=[item.public_payment_id for item in result.items],
                ),
            )
            return
        if len(parts) == 4 and parts[2] in {"approved", "rejected"}:
            status = (
                PaymentSubmissionORM.STATUS_APPROVED
                if parts[2] == "approved"
                else PaymentSubmissionORM.STATUS_REJECTED
            )
            ...
            await message.reply_text(
                ...
                reply_markup=admin_pending_queue_keyboard(
                    ...
                    language,
                    parts[2],
                    [item.public_payment_id for item in result.items],
                ),
            )
            return
        if len(parts) == 4 and parts[2] == "view":
            ...
            if item is None:
                ...
                return
            await message.reply_text(
                _item_text(item, language),
                reply_markup=admin_review_keyboard(item.public_payment_id, language),
            )
            return
        ...
        if query.data and query.data.startswith("cb2:"):
            ...
            consumed = await callback_security.consume(
                ...
                request_id=(
                    context.user_data.get("request_context").request_id
                    if context.user_data.get("request_context")
                    else None
                ),
            )
            ...
            if action_type == "admin.payment.approve":
                result = await service.approve(
                    ...
                    request_id=f"tg:{actor_id}:{payment_id}",
                )
            else:
        ...
        if len(parts) == 4 and parts[2] == "approve":
            ...
            confirm_data = None
            if (
                callback_security is not None
                and context.user_data.get(PLATFORM_USER_KEY) is not None
            ):
            ...
            await message.reply_text(
                ...
                reply_markup=admin_approval_confirmation_keyboard(
                    item.public_payment_id, language, confirm_data
                ),
            )
            return
        if len(parts) == 4 and parts[2] == "reject":
            await message.reply_text(
                t("admin.payments.reject", language=language),
                reply_markup=admin_rejection_reasons_keyboard(parts[3], language),
            )
            return
        if len(parts) == 5 and parts[2] == "reject_reason":
            reason_code, payment_id = parts[3], parts[4]
            if reason_code == "other":
                context.user_data[_ADMIN_REVIEW_KEY] = {
                    "admin_id": actor_id,
                ...
                await message.reply_text(
                    t("admin.payments.enter_custom_reason", language=language, payment=payment_id),
            ...
            await message.reply_text(
                t(
                    "admin.payments.reject_prompt",
                    language=language,
                    payment=payment_id,
                    reason=reason,
                ),
                reply_markup=admin_rejection_confirmation_keyboard(
                    payment_id, language, confirm_data
                ),
            )
            return
...
async def _render_decision(message: Any, result: Any, language: str) -> None:
    if result.is_success:
        decision = result.unwrap()
        if decision.already_decided:
            await message.reply_text(
                t("admin.payments.already_decided", language=language, status=decision.status)
            )
        else:
            await message.reply_text(
                t("admin.payments.decision_success", language=language, decision=decision.decision)
            )
        return
...
async def admin_review_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if (
        message is None
        or actor_id is None
        or state.get("stage") != "custom_reason"
        or state.get("admin_id") != actor_id
    ):
        return
    user = context.user_data.get(PLATFORM_USER_KEY)
    actor_user_id = getattr(user, "id", None)
    authorization = _authorization_service(context)
    if (
        actor_user_id is None
        or authorization is None
        or not await authorization.has_permission_for_user(actor_user_id, "manage_payments")
    ):
        return
    language = _language(context)
    ...
    if not reason:
        await message.reply_text(
            t(
                "admin.payments.enter_custom_reason",
                language=language,
    ...
    await message.reply_text(
        t(
            "admin.payments.reject_prompt",
            language=language,
            payment=state["payment_id"],
            reason=reason,
        ),
        reply_markup=admin_rejection_confirmation_keyboard(state["payment_id"], language),
    )
...
async def admin_review_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_message:
        await update.effective_message.reply_text(
            t("admin.payments.keep_reviewing", language=_language(context))
        )
...
def register(application: Application) -> None:
    ...
    application.add_handler(
        MessageHandler(filters.REPLY & filters.TEXT & ~filters.COMMAND, admin_review_text), group=7
    )
    application.add_handler(
        CallbackQueryHandler(
            admin_payments_callback,
            pattern=r"^admin:(?:home|payments(?::(?:pending|approved|rejected):\d+)?|payments:(?:view|proof|approve|reject):[^:]+|payments:reject_reason:[^:]+:[^:]+)$",
        ),
        group=7,
    )
    application.add_handler(
        CallbackQueryHandler(admin_payments_callback, pattern=r"^cb2:"), group=7
    )
