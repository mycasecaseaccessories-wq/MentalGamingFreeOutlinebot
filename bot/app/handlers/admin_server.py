...
def _sync_service(context) -> OutlineServerSyncService | None:
    registry = context.bot_data.get("registry")
    return None if registry is None else registry.get_or_none(OutlineServerSyncService)


def _selection_service(context) -> ServerSelectionService | None:
    registry = context.bot_data.get("registry")
    return None if registry is None else registry.get_or_none(ServerSelectionService)
...
def _server_text(item, language: str) -> str:
    return t("admin.servers.detail", language=language, public_id=item.public_server_id, name=item.name, display_name=item.display_name or "—", host=item.host or "—", provider=item.provider_type, integration=item.integration_type, location=" / ".join(x for x in (item.country_code, item.country_name, item.region) if x) or "—", status=item.status, health=item.health_status, enabled="✅" if item.enabled else "❌", maintenance="✅" if item.maintenance_mode else "❌", priority=item.priority, weight=item.weight, users=f"{item.current_users}/{item.max_users or '∞'}", keys=item.max_keys or "∞", traffic=f"{item.used_traffic_bytes}/{item.traffic_limit_bytes or '∞'}", checked=_date(item.last_health_check_at), synced=_date(item.last_sync_at), notes=item.notes or "—", latency=item.response_time_ms or "—", inventory=item.existing_key_count if item.existing_key_count is not None else "—", metrics="✅" if item.metrics_available else "—", stale="⚠️ Stale" if item.stale_data else "✅ Fresh", reason=item.health_reason or "—")
...
async def server_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if len(parts) == 5 and parts[2] == "select":
            selector = _selection_service(context)
            if selector is None:
                await message.reply_text(t("admin.servers.selection_unavailable", language=language)); return
            result = await selector.select_server(SelectionRequest(plan=parts[3], country_code=None if parts[4] == "any" else parts[4]))
            if result.selected is None:
                await message.reply_text(t("admin.servers.no_eligible", language=language, reason=result.no_server_reason or "no_eligible_server"), reply_markup=server_menu_keyboard(language)); return
            selected = result.selected
            await message.reply_text(t("admin.servers.selected_preview", language=language, server_id=selected.server_id, country=selected.country_code or "—", score=f"{selected.score.total:.2f}", plan=selected.plan), reply_markup=server_menu_keyboard(language)); return
        if parts == ["admin", "servers", "sync"]:
            sync = _sync_service(context)
            if sync is None: await message.reply_text(t("common.error", language=language)); return
            summary = await sync.sync_all(trigger="admin")
            await message.reply_text(t("admin.servers.sync_result", language=language, attempted=summary.attempted, succeeded=summary.succeeded, failed=summary.failed, skipped=summary.skipped), reply_markup=server_menu_keyboard(language)); return
        if len(parts) == 4 and parts[2] in {"sync", "health", "usage"}:
            sync = _sync_service(context)
            if sync is None: await message.reply_text(t("common.error", language=language)); return
            sid = parts[3]
