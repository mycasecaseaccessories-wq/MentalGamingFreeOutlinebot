...
class Scheduler:
        ...
        *,
        ...
        if job_service is None:
            if sync_service is not None:
                self.add_job(
                    ...
                    "interval",
                    seconds=sync_service.policy.sync_interval_seconds,
                    jitter=30,
                    id="outline-server-sync",
            ...
            if reservation_service is not None:
                self.add_job(
                    reservation_service.expire_reservations,
                    "interval",
                    minutes=1,
                    id="server-reservation-cleanup",
                    replace_existing=True,
                    max_instances=1,
                    coalesce=True,
                )
            if lifecycle_service is not None:
                ...
                sweep = VPNExpirationSweepTask(
                    db=lifecycle_service.db,
                ...
                self.add_job(
                    sweep.run,
                    "interval",
                    seconds=max(60, int(lifecycle_interval_seconds)),
                    id="vpn-expiration-sweep",
                    replace_existing=True,
                    max_instances=1,
                    coalesce=True,
                )
        ...
        if sync_service is not None:
            self.dispatcher.register_handler(
                ...
                lambda _payload: sync_service.sync_all(trigger="scheduled"),
            )
            self._register_periodic(
                BackgroundJobORM.JOB_VPN_LIFECYCLE_SYNC,
                max(60, int(sync_service.policy.sync_interval_seconds)),
            )
        if reservation_service is not None:
            self.dispatcher.register_handler(
                "server_reservation_cleanup",
                lambda _payload: reservation_service.expire_reservations(),
            )
        ...
        if lifecycle_service is not None:
            ...
            self.dispatcher.register_handler(
                BackgroundJobORM.JOB_VPN_EXPIRATION, lambda _payload: sweep.run()
            )
            self._register_periodic(
                BackgroundJobORM.JOB_VPN_EXPIRATION, max(60, int(lifecycle_interval_seconds))
            )
        if order_service is not None:
            self.dispatcher.register_handler(
                BackgroundJobORM.JOB_ORDER_EXPIRATION,
                lambda _payload: order_service.expire_pending_orders(),
            )
            self._register_periodic(BackgroundJobORM.JOB_ORDER_EXPIRATION, 60)
        if free_trial_upgrade_service is not None:
            self.dispatcher.register_handler(
                BackgroundJobORM.JOB_FREE_TRIAL_EXPIRATION,
                lambda _payload: free_trial_upgrade_service.recover_pending_fulfillment(limit=50),
            )
        ...
        if health_service is not None:
            ...
            self.dispatcher.register_handler(
                BackgroundJobORM.JOB_HEALTH_CHECK, _health_check_and_evaluate
            )
            self._register_periodic(BackgroundJobORM.JOB_HEALTH_CHECK, 300)
        if backup_service is not None:
            self.dispatcher.register_handler(
                BackgroundJobORM.JOB_BACKUP_CREATION,
                lambda _payload: backup_service.create_backup(
                    backup_type="automatic", retention_class="daily"
                ),
            )
            self.dispatcher.register_handler(
                BackgroundJobORM.JOB_BACKUP_RETENTION,
                lambda _payload: backup_service.apply_retention(),
            )
            self.dispatcher.register_handler(
                BackgroundJobORM.JOB_BACKUP_RESTORE_TEST,
                lambda _payload: backup_service.run_latest_restore_test(),
            )
        ...
        if maintenance_service is not None:
            self.dispatcher.register_handler(
                BackgroundJobORM.JOB_MAINTENANCE_ACTIVATION,
                lambda _payload: maintenance_service.process_due_windows(),
            )
            self.dispatcher.register_handler(
                BackgroundJobORM.JOB_MAINTENANCE_RECOVERY_CHECK,
                lambda _payload: maintenance_service.maintenance_recovery_snapshot(),
            )
    ...
    def _register_periodic(self, job_type: str, cadence_seconds: int):
        self.add_job(
            ...
            id=f"durable-enqueue-{job_type}",
            replace_existing=True,
            max_instances=1,
            coalesce=True,
            kwargs={"job_type": job_type, "cadence_seconds": cadence_seconds},
        )
    ...
    async def _enqueue_periodic(self, *, job_type: str, cadence_seconds: int):
        ...
        await self.job_service.enqueue(
            job_type=job_type,
            logical_key=f"{job_type}:{bucket}",
            scheduled_for=now,
            payload_safe={"scheduled_bucket": bucket},
        )
