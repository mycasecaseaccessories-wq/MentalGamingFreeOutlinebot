...
class CustomerKeyService(BaseService):
        ...
        async with self.db.session() as session:
            ...
            items = tuple(
                CustomerKeySummary(
                    ...
                    limit_status=getattr(row, "limit_status", "not_configured"),
                    provider_limit_verified=bool(getattr(row, "provider_limit_verified", False)),
                )
                for row in rows
            )
    ...
    async def get_usage_summary(
        ...
        return KeyUsage(
            ...
            provider_limit_verified=detail.provider_limit_verified,
        )
