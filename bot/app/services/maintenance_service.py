...
class MaintenanceService(BaseService):
    ...
    async def _record_control_action(
        self,
        session,
        *,
        actor_id: int,
        ...
        existing = (
            await session.execute(
                select(MaintenanceActionORM).where(
                    MaintenanceActionORM.idempotency_key == idempotency_key
                )
            )
        ).scalar_one_or_none()
        ...
        recent = await session.scalar(
            select(func.count(MaintenanceActionORM.id)).where(
                MaintenanceActionORM.actor_id == actor_id, MaintenanceActionORM.created_at >= cutoff
            )
        )
        if int(recent or 0) >= self._CONTROL_ACTION_LIMIT:
            raise RuntimeError("maintenance_rate_limited")
        safe_metadata = metadata or {}
        row = MaintenanceActionORM(
        ...
        session.add(
            AuditLogORM(
                ...
                entity_id=window_id,
                old_value=None,
                new_value=json.dumps(safe_metadata, sort_keys=True),
                note="Operational maintenance control action",
            )
        )
        return row, False
    ...
    async def get_effective_state(
        self, scope: str | MaintenanceScope, *, at: datetime | None = None
    ) -> dict:
        now = at or self._now()
        scope_value = getattr(scope, "value", scope)
        scopes = (
            [MaintenanceScope.GLOBAL.value, scope_value]
            if scope_value != MaintenanceScope.GLOBAL.value
            else [scope_value]
        )
        ...
        if not rows:
            return {
                "scope": scope_value,
                "state": MaintenanceState.NORMAL.value,
                "source": None,
                "window": None,
            }
    ...
    async def assert_operation_allowed(
        ...
        if not result:
            ...
            raise MaintenanceBlockedError(
                getattr(scope, "value", scope), operation, effective["state"]
            )
    ...
    async def schedule_maintenance(
        ...
        *,
        ...
        status = (
            ...
            if start <= now
            else MaintenanceWindowStatus.SCHEDULED.value
        )
        async with self.db.session() as session:
            await self._record_control_action(
                session,
                actor_id=created_by,
                action="schedule",
                idempotency_key=idempotency_key
                or f"maintenance:schedule:{created_by}:{scope_value}:{start.isoformat()}:{state_value}",
                metadata={"scope": scope_value, "state": state_value, "reason_code": reason_value},
            )
            if status == MaintenanceWindowStatus.ACTIVE.value:
                existing = (
                    await session.execute(
                        select(MaintenanceWindowORM).where(
                            MaintenanceWindowORM.scope == scope_value,
                            MaintenanceWindowORM.status == MaintenanceWindowStatus.ACTIVE.value,
                        )
                    )
                ).scalar_one_or_none()
            ...
            row = MaintenanceWindowORM(
                public_id=f"mw_{uuid4().hex[:20]}",
                scope=scope_value,
                state=state_value,
                status=status,
                ...
                scheduled=start > now,
                alert_suppression_policy=alert_suppression_policy,
                auto_end_policy=auto_end_value,
            )
            session.add(row)
            await session.flush()
        ...
        await bus.emit(
            ...
            if status == MaintenanceWindowStatus.ACTIVE.value
            else EventType.MAINTENANCE_SCHEDULED,
        ...
        async with self.db.session() as session:
            rows = (
                (
                    await session.execute(
                        select(MaintenanceWindowORM).where(
                            MaintenanceWindowORM.status == MaintenanceWindowStatus.SCHEDULED.value,
                            MaintenanceWindowORM.starts_at <= now,
                        )
                    )
                )
            ...
            for row in rows:
                active = (
                    await session.execute(
                        select(MaintenanceWindowORM).where(
                            MaintenanceWindowORM.scope == row.scope,
                            MaintenanceWindowORM.status == MaintenanceWindowStatus.ACTIVE.value,
                        )
                    )
                ).scalar_one_or_none()
        ...
        *,
        ...
        async with self.db.session() as session:
            row = (
                await session.execute(
                    select(MaintenanceWindowORM)
                    .where(MaintenanceWindowORM.public_id == public_id)
                    .with_for_update()
                )
            ).scalar_one_or_none()
            if row is None:
            ...
            await self._record_control_action(
                ...
                actor_id=ended_by,
                action="end",
                idempotency_key=idempotency_key or f"maintenance:end:{ended_by}:{public_id}",
                window_id=row.id,
                metadata={"public_id": public_id, "force": force},
            )
            if not recovery_ok and not force:
                await bus.emit(
                    EventType.MAINTENANCE_RECOVERY_FAILED, public_id=public_id, scope=row.scope
                )
                return {
                    "ended": False,
                    "safe_error_code": "recovery_check_failed",
                    "status": row.status,
                }
    ...
    async def cancel_scheduled_maintenance(
        self, public_id: str, *, cancelled_by: int, idempotency_key: str | None = None
    ) -> dict:
        async with self.db.session() as session:
            row = (
                await session.execute(
            ...
            await self._record_control_action(
                ...
                action="cancel",
                idempotency_key=idempotency_key or f"maintenance:cancel:{cancelled_by}:{public_id}",
                window_id=row.id,
                metadata={"public_id": public_id},
            )
            row.status = MaintenanceWindowStatus.CANCELLED.value
        await bus.emit(
            EventType.MAINTENANCE_CANCELLED, public_id=public_id, cancelled_by=cancelled_by
        )
    ...
    async def resolve_incident(
        ...
        *,
        resolved_by: int = 0,
        resolution_summary: str = "Provider recovered and monitoring is normal.",
    ) -> dict:
        async with self.db.session() as session:
            row = (
                await session.execute(
                    select(OperationalIncidentORM)
                    .where(OperationalIncidentORM.public_id == public_id)
                    .with_for_update()
                )
            ).scalar_one_or_none()
            if row is None:
            ...
            if row.status in {IncidentStatus.RESOLVED.value, IncidentStatus.CLOSED.value}:
                return {
                    "resolved": True,
                    "incident": self._incident_public(row),
                    "idempotent": True,
                }
    ...
    async def list_incidents(self, *, active_only: bool = True, limit: int = 50) -> list[dict]:
        async with self.db.session() as session:
            query = (
                select(OperationalIncidentORM)
                .order_by(OperationalIncidentORM.started_at.desc())
                .limit(min(max(limit, 1), 100))
            )
            if active_only:
                query = query.where(
                    OperationalIncidentORM.status.not_in(
    ...
    async def is_alert_suppressed(
        self, scope: str | MaintenanceScope, alert_key: str, *, at: datetime | None = None
    ) -> dict:
        ...
        return {
            ...
            "window": window,
            "reason": "planned_maintenance" if suppressed else None,
        }
    ...
    async def recovery_check(
        ...
        *,
        ...
        effective = await self.get_effective_state(scope)
        policy_healthy = effective["state"] in {
            MaintenanceState.NORMAL.value,
            MaintenanceState.DEGRADED.value,
        }
        healthy = (
            provider_healthy if provider_healthy is not None else policy_healthy
        ) and queue_healthy
        return {
            "scope": getattr(scope, "value", scope),
            "healthy": healthy,
            ...
            "safe_error_code": None if healthy else "recovery_dependencies_unhealthy",
        }
    ...
    async def process_due_windows(self, *, at: datetime | None = None) -> dict:
            ...
            rows = (
                (
                    await session.execute(
                        select(MaintenanceWindowORM).where(
                            MaintenanceWindowORM.status == MaintenanceWindowStatus.ACTIVE.value,
                            MaintenanceWindowORM.expected_ends_at.is_not(None),
                            MaintenanceWindowORM.expected_ends_at <= now,
                        )
                    )
                )
                .scalars()
        ...
        return {
            ...
            "awaiting_recovery_approval": reminders,
        }
        ...
        async with self.db.session() as session:
            rows = (
                (
                    await session.execute(
                        select(MaintenanceWindowORM).where(
                            MaintenanceWindowORM.status.in_(
                                [
                                    MaintenanceWindowStatus.ACTIVE.value,
                                    MaintenanceWindowStatus.ENDING.value,
                                ]
                            )
                        )
                    )
    ...
    async def get_customer_notice(
        self, scope: str | MaintenanceScope, *, language: str = "en"
    ) -> str | None:
        effective = await self.get_effective_state(scope)
        window = effective.get("window")
        if not window:
        ...
        return t(
            window.get("customer_message_key") or "customer.maintenance.default", language=language
        )
